...
- Continuous-time stationary autocorrelation generated by the random-walk
  Laplacian, exposed as `time_continuous_autocorrelation`.
...
- Cross-platform clean-install smoke tests for the default full-feature
  dependency set, including an end-to-end kNN CLI workflow.

### Changed

- Replaced the former shortest-path-binned spectral covariance reported by
  `calculate_ruggedness_autocorrelation_analytical` with Stadler's stationary
  discrete-time Markov autocorrelation, extended to reversible irregular and
  conductance-weighted graphs. The stochastic estimator now targets the same
  quantity. Legacy values are not numerically comparable with either the new
  discrete-time or continuous-time autocorrelation.
...
- Made the default `landscapy` install cover the `all` and `ml` dependency
  union. The `all` extra remains the exact non-ML union for compatibility.
