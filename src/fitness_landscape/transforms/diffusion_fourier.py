...
def diffusion_fourier_transform(graph: Union[nx.Graph, FitnessLandscape],
                                signal=None,
                                k=None,
                                backend: Literal['numpy, torch'] = 'numpy') -> Union[torch.Tensor, np.ndarray]:
    """
    Compute Markov / diffusion Fourier transform of a signal on the
    graph. The diffusion Fourier transform uses the eigenvectors of the
    transition matrix as the Fourier basis, instead of the graph
    Laplacian. 
...
def _graph_fourier_transform_numpy(graph: Union[nx.Graph, FitnessLandscape],
    ...
    signal : np.ndarray
        The signal to project onto the eigenbasis of the transition
        matrix.
...
def inverse_graph_fourier_transform(eigenvectors: Union[torch.Tensor, np.ndarray],
    ...
    ----------
    eigenvectors : np.ndarray or torch.Tensor
        Eigenvectors of the graph transition matrix.
...
def _inverse_graph_fourier_transform_torch(eigenvectors: Union[np.ndarray, torch.Tensor],
    ...
    """
        Helper function to compute the inverse diffusion FT using the
        torch backend.
